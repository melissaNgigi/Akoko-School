...
// Board member routes
const BOARD_MEMBERS_FILE = path.join(__dirname, '..', 'board_members.json');
...
router.get('/board-members', (req, res) => {
    try {
        const boardMembers = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'data', 'board_members.json')));
        console.log('Sending board members:', boardMembers); // Debug log
...
// Update board member
router.post('/update-board-member', async (req, res) => {
    if (!req.session.isAdmin) {
        return res.status(401).json({ success: false, message: 'Not authenticated' });
    }
    ...
    try {
        ...
        fs.writeFileSync(
            path.join(__dirname, '..', 'board_members.json'),
...
// Update HOD
router.post('/update-hod', async (req, res) => {
    if (!req.session.isAdmin) {
        return res.status(401).json({ success: false, message: 'Not authenticated' });
    }
...
router.post('/update-enrollment', (req, res) => {
    if (!req.session.isAdmin) {
        return res.status(401).json({ 
            success: false, 
            message: 'Not authenticated' 
